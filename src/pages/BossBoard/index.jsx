...
export function BossBoard() {
  ...
  return (
    <section className="space-y-5">
      <div>
        <p className="text-sm font-medium text-emerald-700">部门大盘视图</p>
        <h2 className="mt-2 text-3xl font-semibold tracking-normal text-slate-950">老板视角：多个项目一屏看清</h2>
      </div>
            ...
            return (
              <div key={project.id} className="border-b border-slate-200 last:border-b-0">
                <button onClick={() => setExpandedProject(expandedProject === project.id ? null : project.id)} className="grid w-full gap-4 p-4 text-left md:grid-cols-[1fr_140px_180px_120px_32px] md:items-center">
                  ...
                  <span className="text-sm text-slate-500">{project.tasks?.length || 0} 项任务</span>
                  <ChevronDown className={`text-slate-500 transition ${expandedProject === project.id ? 'rotate-180' : ''}`} size={18} />
                </button>
                {expandedProject === project.id && (
                  <div className="grid gap-2 border-t border-slate-200 bg-slate-50 p-4 md:grid-cols-2 xl:grid-cols-3">
                    {project.tasks?.length ? project.tasks.map((t) => (
                      <div key={t.id} className="rounded-md border border-slate-200 bg-white px-3 py-2">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          ...
                          {t.owner_name ? <span className="text-xs text-slate-500">{t.owner_name}</span> : <span className="text-xs text-slate-500">未认领</span>}
                        </div>
                      </div>
